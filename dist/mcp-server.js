...
server.tool("memory_store", `Store a memory about the current conversation. 
...
server.tool("memory_recall", "Recall relevant memories for a given query. Use this to find past context about a topic.", {
...
server.tool("memory_context", "Get formatted context for the current session. Use this at the start of a conversation to load relevant memories.", {}, async () => {
    const sessionId = await ensureSession();
...
server.tool("memory_headers", "Get lightweight memory headers for quick overview. Use this to see what's stored without loading full details.", {}, async () => {
    const sessionId = await ensureSession();
...
server.tool("memory_goal_create", "Create a goal to track progress. Use this when the user sets an objective.", {
...
server.tool("memory_goal_active", "Get the currently active goal for a session.", {}, async () => {
    const sessionId = await ensureSession();
...
server.tool("memory_goal_complete", "Mark a goal as completed.", {
...
server.tool("memory_session_end", `End the current memory session and create a summary.
...
server.tool("memory_session_create", "Create a new session for tracking conversations.", {
...
server.tool("memory_drill_down", "Get detailed information about a specific memory including its children.", {
...
server.tool("memory_list_sessions", "List all memory sessions to see what's stored.", {}, async () => {
